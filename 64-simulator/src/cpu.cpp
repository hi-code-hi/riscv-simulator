...
u64 pc;			//pc指针
u32 ir;
long count = 0;		//指令计数，指令数可能很大，所以设置成long型
//读内存
void sim_mem_read(u32 address, int size, void *data)
{
	if (address >= MEM_SIZE)
	{
...
//写内存
void sim_mem_write(u32 address, int size, void *data)
{
	if (address >= MEM_SIZE)
	{
		cout << "illegal memory access!" << endl;
		...
		switch(opcode)
		{
			case 0b0010011://整数计算指令
				switch(func1)
				{
						...
						if ((func2 & 0b1111110) == 0b0000000)
						{
							x[rd] = x[rs1] << (I_I_D(ir) & 0x3f);
                                                	#if DEBUG
						...
						x[rd] = (s64)x[rs1] < (s64)I_I_D(ir) ? 1 : 0;
                                                #if DEBUG
					...
					case 0b000://lb
						...
						x[rd] = (s64)((s32)x[rd]);
							...
							<< '(' << "x" << rs1 << '(' << x[rs1] << ')' << ')' << endl;
                                                ...
                                                break;
					 case 0b011://ld
						sim_mem_read(x[rs1]+I_I_D(ir), 8, &x[rd]);
						//x[rd] = (s64)((s32)x[rd]);
                                                #if DEBUG
                                                cout << hex << pc_old << dec <<  '	' << "ld" << '\t'  
							<< "x" << rd << '(' << x[rd] << ')' 
							<< ", " << (s64)I_I_D(ir) 
							<< '(' << "x" << rs1 << '(' << x[rs1] << ')' << ')' << endl;
                                                #endif
				...
				switch(func1)
				{
						...
						sim_mem_write(x[rs1]+S_I_D(ir), 4, &x[rs2]);
							...
							<< '(' << "x" << rs1 << '(' << x[rs1] << ')' << ')' << endl;
                                                ...
                                                break;
					case 0b011://sd
						sim_mem_write(x[rs1]+S_I_D(ir), 8, &x[rs2]);
                                                #if DEBUG
                                                cout << hex << pc_old << dec <<  '	' << "sw" << '\t'
							<< "x" << rs2 << '(' << x[rs2] << ')' 
							<< ", " << (s64)S_I_D(ir) 
			...
			case 0b1101111://非条件跳转指令（UJ类）——JAL
				...
				else if(pc==addr_clock)
				{
					...
					x[10]=clock();
						...
						<< "x" << rd << '(' << x[rd] << ')' 
						<< ", "<< (s64)UJ_I_D(ir)<< '\t' << hex << "pc=" << pc << dec <<endl;
                                #endif
				break;
			case 0b1100111://非条件跳转指令（I类型）——JALR
				x[rd] = pc;
				pc = (x[rs1] + I_I_D(ir)) & 0xfffffffffffffffe;
                                #if DEBUG
                                        cout << hex << pc_old << dec <<  '	' << "jalr" << '\t' 
						<< "x" << rd << '(' << x[rd] << ')' 
						<< ", x" << rs1 << '(' << x[rs1] << ')' 
						<< ", " << (s64)I_I_D(ir)<< '\t' << hex << "pc=" << pc << dec <<endl;
                                #endif
				break;
			...
			case 0b0011011:
				switch(func1)
				{
                                        case 0b000:////ADDIW rd, rs1, imm12
						x[rd] = (s64)((s32)((u32)x[rs1] + (u32)I_I_D(ir)));
                                                #if DEBUG
                                                cout << hex << pc_old << dec <<  '	' << "addiw" << '\t' 
							<< "x"<< rd << '(' << x[rd] << ')' 
							<< ", x"<< rs1 << '(' << x[rs1] << ')' 
							<< ", " << (s64)I_I_D(ir)<< endl;
                                                #endif
					...
					case 0b001:////SLLI rd, rs1, shamt
						if (func2 == 0b0000000)
						{
							x[rd] = (s64)((s32)((u32)x[rs1] << (I_I_D(ir) & 0x1f)));
                                                	#if DEBUG
                                                	cout << hex << pc_old << dec <<  '	' << "slliw" << '\t'  
							<< "x" << rd << '(' << x[rd] << ')' 
							<< ", x" << rs1 << '(' << x[rs1] << ')' 
							<< ", " << (s64)I_I_D(ir) <<endl;
                                                	#endif
					...
					case 0b101://shift right
						switch(func2)
						{
							case 0b0000000://SRLIW rd, rs1, shamt逻辑右移
								x[rd] = (s64)((s32)((u32)x[rs1] >> (I_I_D(ir) & 0x1f)));
                                                        	#if DEBUG
                                                        	cout << hex << pc_old << dec <<  '	' << "srliw"<< '\t' 
								<< "x" << rd << '(' << x[rd] << ')'
								<< ", x" << rs1 << '(' << x[rs1] << ')'
								<< ", " << (s64)I_I_D(ir)<< endl;
                                                        	#endif
								break;
							case 0b0100000://SRAIW rd, rs1, shamt算数右移
								x[rd] = (s64)((s32)((s32)x[rs1] >> (I_I_D(ir) & 0x1f)));
                                                                #if DEBUG
                                                                cout << hex << pc_old << dec <<  '	' << "sraiw"<< '\t'
								<< "x" << rd << '(' << x[rd] << ')' 
								<< ", x" << rs1 << '(' << x[rs1] << ')' 
								<< ", " << (s64)I_I_D(ir)<< endl;
                                                                #endif
			...
			case 0b0111011:
				switch(func1)
				{
					case 0b000:
                                                switch(func2)
                                                {
                                                        case 0b0000000://ADDW rd,rs1,rs2
								x[rd] =(s64)((s32)((u32)x[rs1] + (u32)x[rs2]));
                                                                #if DEBUG
                                                                cout << hex << pc_old << dec <<  '	' << "addw" << '\t'  
								<< "x" << rd << '(' << x[rd] << ')' 
								<< ", x" << rs1 << '(' << x[rs1] << ')' 
								<< ", x" << rs2 << '(' << x[rs2] << ')' 
								<< endl;
                                                                #endif
                                                                break;
                                                        case 0b0100000://SUBW rd,rs1,rs2
								x[rd] = (s64)((s32)((u32)x[rs1] - (u32)x[rs2]));
                                                                #if DEBUG
                                                                cout << hex << pc_old << dec <<  '	' << "subw" << '\t' 
								<< "x" << rd << '(' << x[rd] << ')' 
								<< ", x" << rs1 << '(' << x[rs1] << ')' 
								<< ", x" << rs2 << '(' << x[rs2] << ')' 
								<< endl;
                                                                #endif
                                                                break;
                                                        case 0b0000001://MULW rd, rs1, rs2
                                                                x[rd] = (s64)(((s32)(s32)x[rs1] * (s32)x[rs2]));
                                                                #if DEBUG
                                                                cout << hex << pc_old << dec << '	' << "mulw" << '\t' 
                                                                << "x" << rd << '(' << x[rd] << ')'
                                                                << ", x" << rs1 << '(' << x[rs1] << ')'
                                                                << ", x" << rs2 << '(' << x[rs2] << ')'
                                                                << endl;
						...
						break;
                                        case 0b001://SLLW rd,rs1,rs2
						if (func2 == 0b0000000)
						{
							x[rd] = x[rs1] << (x[rs2] & 0x1f);
                                                        #if DEBUG
                                                        cout << hex << pc_old << dec <<  '	' << "sllw" << '\t' 
								<< "x" << rd << '(' << x[rd] << ')' 
								<< ", x" << rs1 << '(' << x[rs1] << ')' 
								<< ", x" << rs2 << '(' << x[rs2] << ')' 
								<< endl;
					...
					case 0b101:
                                                switch(func2)
                                                {
                                                        case 0b0000000://SRLW rd,rs1,rs2
								x[rd] = (s64)((s32)((u32)x[rs1] >> (x[rs2] & 0x1f)));
                                                        	#if DEBUG
                                                        	cout << hex << pc_old << dec <<  '	' << "srlw" << '\t' 
									<< "x" << rd << '(' << x[rd] << ')' 
									<< ", x" << rs1 << '(' << x[rs1] << ')' 
									<< ", x" << rs2 << '(' << x[rs2] << ')' 
									<< endl;
                                                        	#endif
                                                                break;
                                                        case 0b0100000://SRAW rd,rs1,rs2
								x[rd] = (s64)((s32)((s32)x[rs1] >> (x[rs2] & 0x1f)));
                                                        	#if DEBUG
                                                        	cout << hex << pc_old << dec <<  '	' << "sraw" << '\t' 
									<< "x" << rd << '(' << x[rd] << ')' 
									<< ", x" << rs1 << '(' << x[rs1] << ')' 
									<< ", x" << rs2 << '(' << x[rs2] << ')' 
									<< endl;
                                                        	#endif
                                                                break;
                                                        case 0b0000001://DIVUW rd, rs1, rs2
                                                                x[rd] = (s64)((s32)((u32)x[rs1] / (u32)x[rs2]));
                                                                #if DEBUG
                                                                cout << hex << pc_old << dec <<  '\t' << "divuw" << '\t' 
                                                                        << "x" << rd << '(' << x[rd] << ')'
                                                                        << ", x" << rs1 << '(' << x[rs1] << ')'
                                                                        << ", x" << rs2 << '(' << x[rs2] << ')'
                                                                        << endl;
                                                ...
                                                break; 
					case 0b100:
						if (func2 == 0b0000001)//DIVW rd, rs1, rs2
                                                {
							if(!x[rs2])
							{
								cout<<"DIVW ZERO TRAP!"<<endl;
								exit(0);
                                                                break;
							}
                                                        x[rd] = (s64)((s32)((s32)x[rs1] / (s32)x[rs2]));
                                                        #if DEBUG
                                                        cout << hex << pc_old << dec <<  '\t' << "divw" << '\t' 
                                                                << "x" << rd << '(' << x[rd] << ')'
                                                                << ", x" << rs1 << '(' << x[rs1] << ')'
                                                                << ", x" << rs2 << '(' << x[rs2] << ')'
                                                                << endl;
					...
					case 0b110:
						if (func2 == 0b0000001)//REMW rd, rs1, rs2
                                                {
                                                        x[rd] = (s64)((s32)((s32)x[rs1] % (s32)x[rs2]));
                                                        #if DEBUG
                                                        cout << hex << pc_old << dec <<  '\t' << "remw" << '\t' 
                                                                << "x" << rd << '(' << x[rd] << ')'
                                                                << ", x" << rs1 << '(' << x[rs1] << ')'
                                                                << ", x" << rs2 << '(' << x[rs2] << ')'
                                                                << endl;
					...
					case 0b111:
						if (func2 == 0b0000001)//REMUW rd, rs1, rs2
                                                {
                                                        x[rd] = (s64)((s32)((u32)x[rs1] % (u32)x[rs2]));
                                                        #if DEBUG
                                                        cout << hex << pc_old << dec <<  '\t' << "remuw" << '\t' 
                                                                << "x" << rd << '(' << x[rd] << ')'
                                                                << ", x" << rs1 << '(' << x[rs1] << ')'
                                                                << ", x" << rs2 << '(' << x[rs2] << ')'
                                                                << endl;
